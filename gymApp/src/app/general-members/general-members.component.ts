...
export class GeneralMembersComponent implements OnInit {
	...
	public gymsList: IGyms[];
  	...
  	ngOnInit() {
	    ...
	    this.memberManager.getData('https://vquinonez.github.io/gymApp/gymApp/dist/assets/data/members.json').subscribe( res =>{
	      this.membersList = res;
	      this.formatTableList();
	    });

	    this.gymManager.getData('https://vquinonez.github.io/gymApp/gymApp/dist/assets/data/gyms.json').subscribe( res =>{
	      this.gymsList = res;
	      this.formatTableList();
	    });
  	}
